...
?>
				<br />
                    <div class="radio">
<?php
	$query_answer = mysql_query("SELECT id, id_question, answer FROM answer WHERE id_question = ".$_SESSION["nomor"]);
	while ($row_answer = mysql_fetch_array($query_answer)) {
?>
                        <input id="<?=$row_answer['id'];?>" type="radio" name="answer" value="<?=$row_answer['id'];?>">
